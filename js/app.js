(() => {
    ...
    function showPopup(form, isSucsess, response = "") {
        if (isSucsess) alert("Успешно отправлено"); else alert(`Ошибка ${response.status}. Форма не отправлена`);
        setTimeout((() => {
            if (flsModules.popup) {
    ...
    da.init();
    document.querySelector("[data-open-tg]").addEventListener("click", (e => {
        document.querySelector("#social").classList.add("open-tg");
    }));
    document.querySelector("[data-close-tg]").addEventListener("click", (e => {
        document.querySelector("#social").classList.remove("open-tg");
    }));
    window["FLS"] = true;
    isWebp();
    menuInit();
